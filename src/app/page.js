...
export default function Home() {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center h-screen w-full overflow-hidden">
      ...
      {animationComplete && (
        <>
          <div className="absolute top-20 left-20 w-4 h-4 rounded-full bg-blue-500 opacity-40 animate-ping" style={{ animationDuration: "3s" }}></div>
          <div className="absolute bottom-40 right-40 w-3 h-3 rounded-full bg-purple-500 opacity-30 animate-ping" style={{ animationDuration: "4s" }}></div>
          <div className="absolute top-1/3 right-1/4 w-2 h-2 rounded-full bg-yellow-500 opacity-30 animate-ping" style={{ animationDuration: "5s" }}></div>
        </>
      )}

      {/* Content (no changes) */}
      <div className="z-10 flex flex-col items-center justify-center text-center px-6 max-w-3xl">
        <div
          className={`flex justify-center mb-8 transform transition-all duration-1000 ease-out ${
            loaded ? "opacity-100 translate-y-0 rotate-0" : "opacity-0 -translate-y-16 rotate-12"
          }`}
        >
          <img
            src="/images/ipl_logo.png"
            alt="IPL Logo"
            className={`h-24 ${animationComplete ? "animate-pulse" : ""}`}
        ...
        <div className="overflow-hidden mb-6">
          <h1
            className={`text-4xl md:text-5xl font-bold text-white transition-all duration-1000 delay-300 ease-out ${
        ...
        <div className="overflow-hidden mb-8 max-w-xl">
          <p
            className={`text-xl text-gray-200 transition-all duration-1000 delay-500 ease-out ${
              loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"
            }`}
          >
        ...
        >
          <button
            className="relative bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-lg shadow-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-300 text-lg font-medium hover:shadow-xl transform hover:scale-105 overflow-hidden group"
        ...
        {/* Animated bouncing circle and floating icon remain unchanged */}
      </div>
    </div>
  );
}
